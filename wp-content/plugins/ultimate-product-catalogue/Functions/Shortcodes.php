...
function Insert_Product_Catalog($atts) {
	...
	if ($Product_Search_Text_Label != "") {$Product_Name_Text = $Product_Name_Search_Label;}
		...
		if ($ProductSearch == "namedesc" or $ProductSearch == "namedesccust") {$Product_Name_Text = __("Поиск...", 'UPCP');}
		else {$Product_Name_Text = __("Введите название..", 'UPCP');}
	}	
...
function AddProduct($format, $Item_ID, $Product, $Tags, $AjaxReload = "No", $AjaxURL = "") {
	...
	else {$Details_Text = __("Далее", 'UPCP');}
	...
	if ($format == "List") {				
		...
		$ProductString .= "<div id='prod-cat-title-" . $Product->Item_ID . "' class='prod-cat-title upcp-list-title' onclick='ToggleItem(" . $Product->Item_ID . ");'>" . $Product->Item_Name . " (" . $Product->Item_Price . ")</div>\n";
...
function BuildSidebar($category, $subcategory, $tags, $prod_name) {
	...
	else {$Categories_Text = __("Категории:", 'UPCP');}
	...
	if ($Product_Name_Search_Label != "") {$SearchLabel = $Product_Name_Search_Label;}
		...
		else {$SearchLabel = __("Название инструмента:", 'UPCP');}
	}
